return function(Import)
	...
	function Story:_Refresh(ctx, state)
		...
		if not state.Maps.ByKey[state.Map] then state.Map = Catalog.MapKeys(information, "Story")[1] end
		...
		if not table.find(state.Stages, state.Stage) then state.Stage = state.Stages[1] end
		...
		Init = function(self, ctx)
			...
			state.MapControl = ctx.Registry:Dropdown(section, {
				...
				Callback = function(value)
					state.Map = state.Maps.ByLabel[value] or string.match(tostring(value), "%[([^%]]+)%]$") or tostring(value)
			...
			ctx:RegisterCleanup(ctx.Join:Register("Story", 100, function()
				...
				if not Catalog.QueueUnlocked(info, playerData, queue) then return nil end
				return {Queue = queue, Matchmaking = state.Matchmaking, Delay = state.Delay}
			end))
			local worker = task.spawn(function()
				while state.Alive and ctx.Runtime.Alive do Story:_Refresh(ctx, state) task.wait(2) end
			end)
