return function(Import)
	...
	local function eventQueue(ctx, state)
		...
		if state.Relics <= 0 then state.Draining = false
		elseif state.Draining and relics <= 0 then state.Draining = false
		elseif not state.Draining and relics >= state.Relics then state.Draining = true end
		local act = state.Draining and "Crow" or state.Act
		if not table.find(state.Acts, act) then return nil end
		...
		if not queue.Difficulty or not Catalog.QueueUnlocked(information, playerData, queue) then return nil end
		if act == "Crow" and relics <= 0 then return nil end
		return queue
	end
	...
	return {
		...
		Init = function(self, ctx)
			local information = ctx.Game:Information() or {}
			local maps = Catalog.MapKeys(information, "VillainInvasion")
			local state = {Alive = true, Enabled = false, Matchmaking = false, Delay = 1, Relics = 0, Draining = false, Map = maps[1]}
			state.Acts = Catalog.Acts(information, "VillainInvasion", state.Map)
			state.Act = state.Acts[1]
			for _, act in ipairs(state.Acts) do if act ~= "Crow" then state.Act = act break end end
			...
			ctx.Registry:Slider(section, {
				Name = "Farm Crow at Relics (0=off)", Default = 0, Minimum = 0, Maximum = 200, Precision = 0,
				Callback = function(value) state.Relics = math.floor(value) if state.Relics <= 0 then state.Draining = false end end,
			...
			local worker = task.spawn(function() while state.Alive and ctx.Runtime.Alive do Event:_Refresh(ctx, state) task.wait(2) end end)
