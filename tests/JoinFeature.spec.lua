...
local information = {
	Maps = maps,
	ChallengeInfo = {
		Info = {Regular = {Amount = 1, RefreshTime = 1800}, Daily = {Amount = 1, RefreshTime = 86400}, Weekly = {Amount = 1, RefreshTime = 604800}},
		IsChallengeAvailable = function() return true end,
	},
	StageDrops = {GetDrops = function() return {{Asset = "Gem"}} end},
	Items = {Gem = {DisplayName = "Gems"}},
}
local playerData = {CompletedMaps = {}, ChallengeData = {ClearHistory = {}, DailyClearHistory = {}}, ItemData = {CrowRelic = 0}}
local challengeData = {
...
function section:Label(settings) return {Settings = settings} end
...
local context = {
	...
	Game = {
		...
		State = function(_, name) if name == "ChallengeData" then return challengeData end end,
		IsInGame = function() return false end,
	},
	Join = {Register = function(_, name, priority, provider) providers[name] = provider return function() providers[name] = nil end end},
...
for flag, current in pairs(controls) do
	if string.find(flag, "map") or string.find(flag, "act") or string.find(flag, "stage") or string.find(flag, "difficulty") or string.find(flag, "types") or string.find(flag, "drop") then
