...
return {
	...
	OnEnable = function(state, runtime, hub)
		...
		runtime.BootThread = task.spawn(function()
			local ReplicatedStorage = game:GetService("ReplicatedStorage")
			local Players = game:GetService("Players")
			runtime.LocalPlayer = Players.LocalPlayer

			local okRequire, errRequire = pcall(function()
				runtime.Nodes = require(ReplicatedStorage:WaitForChild("Nodes"))
				runtime.Information = require(ReplicatedStorage:WaitForChild("Shared"):WaitForChild("Information"))
				local FusionPackage = ReplicatedStorage:WaitForChild("FusionPackage")
				local Fusion = require(FusionPackage:WaitForChild("Fusion"))
				runtime.Dependencies = require(FusionPackage:WaitForChild("Dependencies"))
				runtime.Actions = require(FusionPackage:WaitForChild("Actions"))
			...
			if typeof(runtime.Replica.OnChange) == "function" then
				runtime.ChangeConnection = runtime.Replica:OnChange(function(_action, path)
					if not runtime.Running then
						return
					end
			...
			scanAndClaim()
			runtime.PollThread = task.spawn(function()
